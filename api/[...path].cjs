...
module.exports = async function apiProxyHandler(req, res) {
  try {
    ...
    // When invoked via vercel.json rewrite:
    //   /api/<anything>  ->  /api/[...path].cjs?path=<anything>
    // We reconstruct the original path and preserve remaining query params.
    let targetUrl;
    if (rewrittenPath !== null) {
      incomingUrl.searchParams.delete("path");
      const cleaned = String(rewrittenPath).replace(/^\/+/, "");
      const proxiedPath = cleaned ? `/api/${cleaned}` : "/api/";
      targetUrl = `${baseUrl}${proxiedPath}${incomingUrl.search}`;
    } else {
      // Direct invocation (should be uncommon, but supported).
      // `req.url` includes `/api/...` + querystring. We proxy it 1:1.
