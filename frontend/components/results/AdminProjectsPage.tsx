...
import { adminListProjects } from "@/lib/results-api";
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="rounded-2xl bg-white px-5 py-3 text-center border border-[var(--border-default)] shadow-[var(--shadow-sm)]">
                <div className="text-2xl font-semibold text-[var(--text-primary)]">{totalSessions}</div>
                <div className="text-xs text-[var(--text-muted)] font-medium">Total Sessions</div>
              </div>
                ...
                <div className="mt-4 rounded-xl bg-[var(--bg-secondary)] p-4 border border-[var(--border-default)]">
                  ...
                  <ul className="space-y-2 text-sm text-[var(--text-secondary)]">
                    ...
                    <li className="flex items-center gap-2">
                      ...
                      <code className="rounded bg-[var(--bg-primary)] px-1.5 py-0.5 text-xs border border-[var(--border-default)]">ADMIN_LOCAL_KEY</code>{" "}
                      configured
                    </li>
                  </ul>
                </div>
      ...
      {q.data && (
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 stagger-children">
          {q.data.projects.map((p) => (
            <Link key={p.id} href={`/results/projects/${p.id}`} className="group block">
              <div className="glass-card rounded-3xl overflow-hidden transition-all-slow hover:-translate-y-1 hover:shadow-[var(--shadow-lg)]">
                <div className="p-5">
                  <div className="mb-4">
                    ...
                    <p className="text-xs text-[var(--text-muted)] font-mono truncate mt-1">{p.id}</p>
                  </div>
                  ...
                  <div className="rounded-2xl bg-[var(--bg-secondary)] p-3 border border-[var(--border-default)]">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xl font-semibold text-[var(--text-primary)]">{p.session_count}</span>
                    </div>
                  </div>
