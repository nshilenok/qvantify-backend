...
export function AdminProjectPage() {
  ...
  const normalizeAdminLike = React.useCallback((value?: number | null) => {
  ...
  const audioTokens = usageTotals?.audio ?? 0;
  const otherTokens = usageTotals?.other ?? 0;
  const audioRate = usageQ.data?.rate_usd_per_1k_audio ?? usdRate;
  const totalUsd =
  ...
  const audioUsd =
    usageTotalsUsd?.audio ?? (audioTokens / 1000) * audioRate;
  ...
  const audioPct = totalTokens > 0 ? (audioTokens / totalTokens) * 100 : 0;
  ...
  const sessionTokenLabel = React.useMemo(() => {
    const total = records.reduce((sum, record) => sum + (record.audio_tokens || 0), 0);
  ...
  React.useEffect(() => {
    if (!selected) return;
    setOptimisticLike(normalizeAdminLike(selected.admin_like));
    setLikeStatus("idle");
    if (likeStatusTimer.current) window.clearTimeout(likeStatusTimer.current);
  ...
  const currentLike = optimisticLike ?? normalizeAdminLike(selected?.admin_like);
  ...
  const getPublicBase = React.useCallback(() => {
    const configured = (process.env.NEXT_PUBLIC_QVANTIFY_BASE_URL || "").replace(/\/$/, "");
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <div className="flex items-center gap-2 rounded-xl border border-[var(--border-default)] bg-[var(--bg-secondary)] px-3 py-2 text-xs text-[var(--text-secondary)]">
                      ...
                      <span className="font-semibold text-[var(--text-primary)]">
                        {usageQ.isLoading || usageQ.error
                          ? "—"
                          : `${formatTokenCount(audioTokens)} • ${formatUsd(audioUsd)}`}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  {!usageQ.isLoading && !usageQ.error && (
                    <>
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-3 text-[11px] text-[var(--text-muted)]">
                        ...
                        {audioTokens > 0 && <span>Audio {Math.round(audioPct)}%</span>}
